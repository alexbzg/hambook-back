...
from app.api.dependencies.qso_logs import get_post_for_update, get_post_for_view, get_visibility_level
...
async def create_post(*,
	...
	postss_repo: PostsRepository = Depends(get_repository(PostsRepository)),    
) -> PostPublic:
    ...
    created_post = await qso_posts_repo.create_post(new_post=new_post, requesting_user=current_user)
...
async def delete_post(*,
    post_id: int,
	current_user: UserInDB = Depends(get_current_active_user),
	posts_repo: QsoLogsRepository = Depends(get_repository(QsoLogsRepository)),    
    post: QsoLogInDB = Depends(get_post_for_update)
) -> dict:
...
async def posts_query_by_user(*,
    ...
    visibility: PostVisibility = Depends(get_visibilty_level)
	posts_repo: PostsRepository = Depends(get_repository(PostsRepository)),    
) -> List[PostPublic]:
...
@router.get("/{post_id}", response_model=PostPublic, name="postss:query-by-post-id")
